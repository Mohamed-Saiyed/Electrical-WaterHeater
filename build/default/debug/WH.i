...
# 4 "C:/Program Files (x86)/Microchip/MPLABX/v5.40/packs/Microchip/PIC16Fxxx_DFP/1.2.33/xc8\pic\include\__size_t.h"
...
extern uint32_t Ten_average_temp;
...
if(SW_GetState(SW_PLUS) == SW_PRE_PRESSED && Init_flag == 1)
{
if(set_temp < 75)
{
set_temp += (5);
...
else if(SW_GetState(SW_MINUS) == SW_PRE_PRESSED && Init_flag == 1)
{
if(set_temp > 35)
{
set_temp -= (5);
...
void WH_Save_Last_Temp(void)
{
EEPROM_Write(0x0030 , set_temp);
}

void WH_Turn_On_Off(void)
{
if(Ten_average_temp <= (set_temp - 5))
{
WH_turn_on();
COOL_turn_off();
}
else if(Ten_average_temp >= (set_temp + 5))
{
WH_turn_off();
COOL_turn_on();
}
...
{
if(((SW_GetState(SW_PLUS) == SW_PRE_PRESSED) ||
(SW_GetState(SW_MINUS) == SW_PRE_PRESSED)) && (Init_flag !=1) && (Sleep_flag != 0))
{
WH_turn_off();
COOL_turn_off();
MODE = Set_Mode;
